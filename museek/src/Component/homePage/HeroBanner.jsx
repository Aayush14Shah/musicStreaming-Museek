...
const HeroBanner = ({ featured, forceWhiteText = false }) => {
  ...
  return (
    <div className="relative w-full h-64 sm:h-80 md:h-96 bg-[var(--bg-secondary)] text-[var(--text-primary)] flex items-center justify-center overflow-hidden z-10 mb-8">
    <WaveBackground className="absolute inset-0 z-0" />
    <div className={`relative z-20 flex flex-col items-center justify-center text-center px-4 md:px-8 max-w-4xl mx-auto ${forceWhiteText ? 'text-white' : ''}`}>
      <h1 className="text-3xl sm:text-4xl md:text-6xl font-bold mb-4">{banner.name}</h1>
      <p className="text-sm sm:text-base md:text-xl max-w-2xl mx-auto mb-6 text-[var(--accent-primary)] line-clamp-3">{banner.description}</p>
