...
require('dotenv').config({ path: './museek-backend/.env' });
...
async function testSpotifyAPI() {
  try {
    ...
    // Step 2: Test Track API with known tracks
    console.log('\n🎵 Step 2: Testing Track API...');
    ...
    const testTracks = [
      ...
      { id: '7qiZfU4dY1lWllzX7mPBI3', name: 'Shape of You (Alternative)' }
    ];
    ...
    for (const track of testTracks) {
      try {
        ...
        if (trackData.preview_url) {
          ...
          console.log(`   🎧 Preview available: YES`);
        } else {
          console.log(`   ❌ Preview URL: null`);
          console.log(`   🎧 Preview available: NO`);
        }
